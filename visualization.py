...
def wav_to_spectrogram(wav_filepath, data_folderpath): 
    ...
    """
    # TODO: add options to modify fft (with defaults)
    ...
    with wave.open(wav_filepath, "rb") as wav_file:
        ...
        #audio_data_clip = audio_data.T[0]
    
    if (len(audio_data) == 1): # use channel count?
        frequencies, times, spectrogram = signal.stft(x=audio_data[0], fs=sample_rate)#, return_onesided=True)#, mode='complex')
        ...
        frequencies_ch1, times_ch1, spectrogram_ch1 = signal.stft(x=audio_data[0], fs=sample_rate)#, return_onesided=True)#, mode='complex')
        fig_ch1 = plt.figure(frameon=False, figsize=(img_width, img_height), dpi=1000)
        im_1 = plt.pcolormesh(times_ch1, frequencies_ch1, np.log2(spectrogram_ch1 + 1), shading='auto')
        plt.axis('off')
        plt.close()
        ...
        frequencies_ch2, times_ch2, spectrogram_ch2 = signal.spectrogram(x=audio_data[1], fs=sample_rate, return_onesided=True)#, mode='magnitude')
...
wav_to_MFCC("data/raw/10 Africa.wav", "data/MFCCs")
